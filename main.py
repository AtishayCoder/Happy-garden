...
def draw():
    ...
    if not game_over:
        ...
        screen.draw.text(f"Garden happy for: {time_elapsed} seconds", topleft=(10, 10), color="black")
    else:
        if not finalized:
            cow.draw()
            screen.draw.text(f"Garden happy for: {time_elapsed} seconds", topleft=(10, 10), color="black")
...
def mutate():
    ...
    if not game_over:
        rand_flower = randint(0, len(flower_list) - 1)
        fangflower = flower_list[rand_flower]
        fangflower.image = "fangflower"
        flower_list.pop(rand_flower)
...
# Use pgzrun main.py to play! Keep those flowers from wilting and be safe from the naughty fangflowers!
